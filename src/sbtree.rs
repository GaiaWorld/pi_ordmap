use std::rc::Rc;
#[macro_export]
macro_rules! custom_ref {
    ($x:ident) => {
        use crate::ordmap::{ActionResult, ActionResultType, Entry, ImOrdMap, Iter};
        ...
        impl<K: TreeByteSize + Ord + Clone, V: TreeByteSize + Clone> Node<K, V> {
            ...
            fn actions<F>(node: &Tree<K, V>, func: &mut F) -> Option<Tree<K, V>>
            where
                F: FnMut(&Entry<K, V>) -> ActionResult<V>,
            {
                match node {
                    Some(n) => {
                        let result = func(&n.entry);
                        let (left, right) = match Self::actions(&n.left, func) {
                            Some(l) => match Self::actions(&n.right, func) {
                                Some(r) => (l, r),
                                None => (l, n.right.clone()),
                            },
                            None => match Self::actions(&n.right, func) {
                        ...
                        let root = match result {
                            ...
                            ActionResult::Upsert(val) => Node::new(
                                1 + r_size + l_size,
                                left,
                                Entry(n.entry.0.clone(), val),
                                right,
                            ),
                        };
        ...
        impl<K: TreeByteSize + Ord + Clone, V: TreeByteSize + Clone> ImOrdMap for Tree<K, V> {
            ...
            fn upsert(&self, key: K, value: V, copy: bool) -> (Option<Option<V>>, Self) {
                ...
                match self {
                    ...
                    _ => (
                        None,
                        new_tree(Node::new(1, None, Entry::new(key, value), None)),
                    ),
                }
            }
            ...
            fn action<F>(&self, key: &K, func: &mut F) -> Option<(ActionResultType, Self)>
            where
                F: FnMut(Option<&V>) -> ActionResult<V>,
            {
                match self {
                    &Some(ref node) => match key.cmp(&node.entry.0) {
                        Ordering::Greater => match node.right.action(key, func) {
                            Some((ActionResultType::Insert, r)) => Some((
                            ...
                            Some((ActionResultType::Update, r)) => Some((
                            ...
                            Some((ActionResultType::Delete, r)) => Some((
                        ...
                        _ => match func(Some(&node.entry.1)) {
                            ActionResult::Upsert(r) => Some((
                                ActionResultType::Update,
                                new_tree(Node::new(
                                    node.size,
                                    node.left.clone(),
                                    Entry::new(key.clone(), r),
                                    node.right.clone(),
                                )),
                            )),
                    ...
                    _ => match func(None) {
                        ActionResult::Upsert(r) => Some((
                            ActionResultType::Insert,
                            new_tree(Node::new(1, None, Entry::new(key.clone(), r), None)),
                        )),
                        _ => None,
                    },
                }
            ...
            fn map<F>(&self, func: &mut F) -> Self
            where
                F: FnMut(&Entry<Self::Key, Self::Val>) -> ActionResult<Self::Val>,
            {
        ...
        impl<'a, K: 'a + Clone + Ord, V: 'a + Clone> Iter<'a> for Tree<K, V> {
            ...
            fn iter(&self, key: Option<&K>, descending: bool) -> Self::IterType {
                ...
                match self {
                    Some(ref node) => match key {
                        Some(k) => {
                            match descending {
                                true => {
                                    it.len = some_down_key_r(&mut it.arr, node, k, it.len);
                                    it.next_fn = IterTree::next_descending;
                                }
                        ...
                        None => {
                            ...
                            match descending {
                                true => {
                                    it.len += down_r(&mut it.arr, node, it.len);
                                    it.next_fn = IterTree::next_descending;
                                }
        ...
        fn creat_node<K: Ord + Clone, V: Clone>(
            arr: &mut Vec<Entry<K, V>>,
            start: usize,
            len: usize,
        ) -> Node<K, V> {
            let r_size = (len - 1) / 2;
        ...
        fn down_r<'a, K: Clone, V: Clone>(
            arr: &mut [*const Node<K, V>; 32],
            last: &'a Node<K, V>,
            index: usize,
        ) -> usize {
            match last.right {
        ...
        fn down_key_l<'a, K: Ord + Clone, V: Clone>(
            arr: &mut [*const Node<K, V>; 32],
            last: &'a Node<K, V>,
            key: &K,
            index: usize,
            ...
            match last.left {
                Some(ref v) => some_down_key_l(arr, v, key, index),
                None => 0,
            }
        }

        fn down_key_r<'a, K: Ord + Clone, V: Clone>(
            arr: &mut [*const Node<K, V>; 32],
            last: &'a Node<K, V>,
            key: &K,
            index: usize,
            ...
            match last.right {
                Some(ref v) => some_down_key_r(arr, v, key, index),
                None => 0,
            }
        }

        fn some_down_key_l<'a, K: Ord + Clone, V: Clone>(
            arr: &mut [*const Node<K, V>; 32],
            v: &'a Node<K, V>,
            key: &K,
            index: usize,
            ...
            match key.cmp(&v.entry.0) {
                ...
                Ordering::Greater => match v.right {
        ...
        fn some_down_key_r<'a, K: Ord + Clone, V: Clone>(
            arr: &mut [*const Node<K, V>; 32],
            v: &'a Node<K, V>,
            key: &K,
            index: usize,
...
pub fn test_sbtree() {
    ...
    let mut i = 100;
    for v in Iter::iter(&mut tree, None, true) {
        //降序迭代
        //print!("{},", v.0);
        assert_eq!(v.0, i);
        i -= 1;
    ...
    let mut i = 50;
    for v in Iter::iter(&mut tree, Some(&50), false) {
        //从指定键升序迭代
        //print!("{},", v.0);
        assert_eq!(v.0, i);
        i += 1;
    ...
    let mut i = 50;
    for v in Iter::iter(&mut tree, Some(&50), true) {
        //从指定键降序迭代
        assert_eq!(v.0, i);
        i -= 1;
    }
